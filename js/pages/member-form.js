...
export class MemberFormController {
    ...
    createActionButtons() {
        ...
        if (this.mode === 'view') {
            ...
            // Changed order - Save first, then Cancel
        ...
        window.parent.postMessage({ type: 'MEMBER_FORM_LOADED' }, '*');
    }
    ...
    async saveMember() {
        ...
        try {
            ...
            if (result.success) {
                this.notifyParent('MEMBER_FORM_SAVED');
                this.closeForm();
            } else {
    ...
    closeForm() {
        this.notifyParent('MEMBER_FORM_CLOSE');
    }
